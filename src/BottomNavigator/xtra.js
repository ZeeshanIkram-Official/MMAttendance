...
// const targetLat = 28.415674763844716;
// const targetLng = 70.30659125124382;
...
const HomeScreen = ({ navigation }) => {
  ...
  const loadData = async () => {
    try {
      ...
      console.log('📅 Current Date:', currentDate, 'Saved Date:', savedDate);

      if (savedAttendance) {
        const state = JSON.parse(savedAttendance);
        console.log('🔄 Restoring State:', state);
        if (savedDate !== currentDate && state.isCheckedOut) {
          console.log('🔄 Resetting state for new day');
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      if (distance > 100) {
        showCustomAlert('error', 'Error', 'You must be within 100 meters of the office to check-in!');
        setIsCheckInLoading(false);
        return;
      }
                ...
                {isCheckedIn && !isCheckedOut ? (
                  activities
                    .filter(activity => {
                      ...
                      const activityTime = parseTimeString(activity.time);
                      const checkIn = parseTimeString(checkInTime);
                      const checkOut = parseTimeString(checkOutTime) || new Date();
                      return activityTime >= checkIn && activityTime <= checkOut;
                    })
                    .length > 0 ? (
                    activities
          ...
          >
            <View style={[styles.modalContent, { maxHeight: SCREEN_HEIGHT * 0.8 }]}>
              ...
              >
                ...
                <TextInput
                  style={[styles.textInput, { marginBottom: 150 }]}
                  multiline
